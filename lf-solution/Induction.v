...
(** **** Exercise: 2 stars, advanced, recommended (plus_comm_informal)  
    ...
    Proof: by inductions of [n].

    - First, suppose [n = 0]. We must show

        0 + m = m + 0

      This follows directly from Theorem [plus_n_0].
...
Theorem mult_Sn_r : forall m n : nat,
  m * (S n) = m + m * n.
Proof.
  intros m n.
...
Theorem leb_refl : forall n:nat,
  true = (n <=? n).
Proof. (* destruct // actually, induction *)
  induction n as [| n' IHn'].
  - reflexivity.
  - simpl.
    rewrite <- IHn'.
...
Example bin_to_nat_to_bin_not_pres : nat_to_bin (bin_to_nat (A Z)) = Z.
Proof. reflexivity. Qed.

(* Because you can add as many leading zeros as you want. In this case,
   [(A Z)] as well as [Z] with an arbitrary number of constructor [A] in
   front is equivalent to [Z]. They might not be preserved during the
   conversion. *)
...
Example test_normalize2 : (normalize (A (B (A (A Z))))) = (A (B Z)).
Proof. simpl. reflexivity. Qed.

(* Theorem double_btn_aa : forall (a b : bin),
  (b = A a) -> ((bin_to_nat a) + (bin_to_nat a) = bin_to_nat a).
Proof.
  intros a b H. *)

Fixpoint eq_Z (b : bin) : bool :=
  match b with
  | Z => true
  | A b' => eq_Z b'
  | B b' => false
