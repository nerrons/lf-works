...
Proof.
  ...
  - destruct m.
    + reflexivity.
    + rewrite plus_comm in H. (* No idea why this works. *)
      discriminate.
Qed.
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R [HPQ1 HPQ2] [HQR1 HQR2].
  split.
  - intros HP.
    apply HQR1.
    apply HPQ1.
...
Proof.
  ...
  - intros HPQR. split.
    ...
    + destruct HPQR.
      ...
      * destruct H as [H1 H2].
        right. apply H2.
  - intros HPQPR. destruct HPQPR as [H1 H2].
    destruct H1.
    * left. apply H.
    * destruct H2.
      + left. apply H0.
...
Proof.
  ...
  - intros HPQ.
    destruct HPQ as [e H].
    destruct H as [H | H].
    + left. exists e. apply H.
    + right. exists e. apply H.
  - intros HPQ.
    destruct HPQ as [H | H].
    + destruct H as [e H]. exists e. left. apply H.
    + destruct H as [e H]. exists e. right. apply H.
Qed.
(** [] *)
  ...
  - split.
    + simpl. intros H. destruct H as [H | H].
      ...
      * apply IH in H.
        destruct H as [m Hm].
        exists m.
        destruct Hm as [H0 H1].
        split.
        { apply H0. }
        { right. apply H1. }
    + simpl. intros H. destruct H as [m [Hm0 [Hm1 | Hm1]]].
      * left. rewrite Hm1. apply Hm0.
      * rewrite IH.
        right.
        exists m. split.
        { apply Hm0. }
        { apply Hm1. }
Qed.
(* I suspect this doesn't need to be this long *)
...
Lemma rev_app_l2 : forall X (l1 l2 : list X),
...
Proof.
  ...
  induction l1 as [| h t IH].
  + intros l2. split.
    - simpl. unfold is_empty. destruct l2.
      * reflexivity.
      * discriminate.
    - intros H0.
      rewrite <- H0.
  ...
  + intros lx. split.
    ...
    - intros Hht.
      destruct lx as [| hx tx].
      * discriminate Hht.
      * simpl. injection Hht. intros Ht Hh.
        apply andb_true_iff. split.
        { rewrite Hh. rewrite H. reflexivity. }
        { apply IH. apply Ht. }
Qed.
(** [] *)
...
Proof.
  ...
  + induction l as [| h t IH].
    ...
    - simpl. intros H.
      rewrite andb_true_iff in H. destruct H as [H0 H1].
      split.
      * apply H0.
      * apply IH. apply H1.
  + induction l as [| h t IH].
