...
Proof.
  intros n. split.
  + intros E. induction E as [| | n m Hn Hx Hm IH].
    - apply ev_0.
  ...
  + intros E. induction E as [| n' E' IH].
    - apply even'_0.
    - assert (Hss: (S (S n')) = 2 + n'). { reflexivity. }
      rewrite Hss.
      apply even'_sum.
      * apply even'_2.
      * apply IH.
...
Proof.
  ...
  - intros m. simpl. intros H. apply IH.
    inversion H.
    apply H1.
Qed.
...
Example test_total_relation_0 : total_relation 0 5.
Proof. apply tr. Qed.

Example test_total_relation_1 : total_relation 99 599.
Proof. apply tr. Qed.
...
Inductive empty_relation : nat -> nat -> Prop :=
  | er n m : False -> empty_relation n m.

Example test_empty_relation_0 : ~ (empty_relation 0 5).
Proof. unfold not. intros E. inversion E. apply H. Qed.
...
Proof.
  ...
  inversion Emn as [| n' E' H].
  - apply Eno.
  - induction Eno as [| o' E2 IH].
    + apply Emn.
    + apply le_S. apply IH.
Qed.
...
Proof.
  ...
  - intros m E. apply O_le_n.
  - intros mx E. inversion E.
    + apply le_n.
    + apply (le_trans (S n') (S (S n')) mx).
...
Proof.
  intros n.
  induction n as [| n' IH].
  - intros m E. reflexivity.
  - intros m E. destruct m as [| m'].
    + inversion E.
    + simpl. 
      apply IH.
      apply Sn_le_Sm__n_le_m in E.
      apply E.
Qed.
...
(** - Which of the following propositions are provable?
    ...
    - R 1 1 2 is provable while R 2 2 6 is not.
    - No. m and n are symmetrical anyways. 
    - No. c4 is identical to applying c2 and c3.
*)
...
Proof.
  ...
  + intros E. induction E.
    ...
    - inversion IHE. rewrite plus_comm in H0. inversion H0.
      apply plus_comm.
    - rewrite plus_comm. apply IHE.
  + intros E. generalize dependent m. generalize dependent n.
    induction o as [| o' IH].
    - intros m n H. destruct n.
      * simpl in H. rewrite H. apply c1.
      * discriminate H.
    - intros mx nx H. destruct nx.
      * simpl in H. rewrite H. apply c3. apply IH. reflexivity.
      * apply c2. apply IH. simpl in H. inversion H. reflexivity.
Qed.
(* this proof also shows that c4 and c5 are redundant. *)
...
| ss_e l : subseq [ ] l
| ss_1 x xs l (H: subseq xs l) : subseq xs (x :: l)
| ss_2 x xs l (H: subseq xs l) : subseq (x :: xs) (x :: l).
(* for constructors I can defined whatever arguments i want *)
...
Theorem subseq_trans : forall (l1 l2 l3 : list nat),
  ...
  subseq l1 l3.
Proof.
  intros. generalize dependent l1. induction H0 as [| x c2 c3 E' | x c2 c3 E'].
  + intros l1 E12. inversion E12. apply ss_e.
  + intros l1 E12. apply ss_1. apply IHE'. apply E12.
  + intros l1 E12. inversion E12.
    - apply ss_e.
    - apply ss_1, IHE', H1.
    - apply ss_2, IHE', H1.
Qed.
...
(* first and second.
   The nat cannot exceed the largest element in the list
...
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp),
  ...
  fold app ss [] =~ Star re.
Proof.
  intros T ss re E. induction ss as [| h t IH].
  + simpl. apply MStar0.
  + simpl. apply MStarApp.
    - apply E. simpl. left. reflexivity.
    - apply IH. intros ls H. apply E. simpl. right. apply H.
Qed.
...
Proof.
  ...
  + generalize dependent s1. induction s2 as [| h2 t2 IH].
    - intros l E. simpl in E. inversion E. reflexivity.
    - Abort.
(* HOW????????????????????????????? *)
...
Proof.
  ...
  + intros H. induction re.
    ...
    - simpl in H. apply andb_true_iff in H. destruct H as [H1 H2].
      destruct IHre1 as [s1 IH1]. { apply H1. }
      destruct IHre2 as [s2 IH2]. { apply H2. }
      exists (s1 ++ s2). apply MApp.
      * apply IH1. * apply IH2.
    - simpl in H. apply orb_true_iff in H. destruct H as [H1 | H2].
      * destruct IHre1 as [s1 IH1]. { apply H1. }
        exists s1. apply MUnionL. apply IH1.
      * destruct IHre2 as [s2 IH2]. { apply H2. }
        exists s2. apply MUnionR. apply IH2.
    - exists []. apply MStar0.
Qed.
(** [] *)
...
Lemma MStar'' : forall T (s : list T) (re : reg_exp),
  ...
  exists ss : list (list T),
    ...
    /\ forall s', In s' ss -> s' =~ re.
Proof.
  intros T s re Hm. remember (Star re) as re'.
  induction Hm.
  - discriminate.
  - discriminate.
  - discriminate.
  ...
  - exists []. split.
    + reflexivity.
    + intros s' H. simpl in H. destruct H.
  - inversion Heqre'. apply IHHm2 in Heqre'.
    destruct Heqre' as [ssx [H2 Hin]] eqn:Eeq.
    exists ([s1] ++ [s2]). split.
    + simpl. rewrite app_nil_r. reflexivity.
    + simpl. intros sx H. destruct H.
      * rewrite <- H. apply Hin. Abort.
...
Proof.
  ...
  - (* MEmpty *)
    simpl. omega.
  - simpl. omega.
  - simpl. intros Hs. rewrite app_length in Hs.
    Abort.
(* This one is definitely too hard for me; will come back later. *)
...
Proof.
  ...
  + intros _ H. simpl in H. destruct H.
  + simpl. destruct (eqbP n h) as [H | H].
    - intros H0. discriminate H0.
    - simpl. intros H0 [Hh | Hh].
      * unfold not in H. symmetry in Hh. apply H. apply Hh.
      * unfold not in IH. apply IH.
        { apply H0. }
        { apply Hh. }
Qed.
...
Definition not_head {X: Type} (x: X) (l: list X) : Prop :=
  match l with
  | [] => True
  | h :: t => x <> h
  end.
...
| nostutter_add x l (H: not_head x l) (I: nostutter l): nostutter (x :: l)
.

(* actually cant believe I've done this. *)
...
| im_0 : is_merge [ ] [ ] [ ]
| im_1 (x : X) l1 l2 l (H: is_merge l1 l2 l) : is_merge (x :: l1) l2 (x :: l)
| im_2 (x : X) l1 l2 l (H: is_merge l1 l2 l) : is_merge l1 (x :: l2) (x :: l)
.

Theorem filter_merge : forall (X : Type) (test : X -> bool) (l1 l2 l: list X),
  is_merge l1 l2 l
...
Proof.
  ...
  + reflexivity.
  + simpl. rewrite H0.
    - apply IHis_merge in H1.
      * rewrite H1. reflexivity.
      * intros. apply H0. simpl. right. apply H2.
    - simpl. left. reflexivity.
  + simpl.
    destruct (test x) eqn:Etx.
    - assert (Hcon: test x = false). {
        apply H1. simpl. left. reflexivity.
      }
      rewrite Etx in Hcon. discriminate Hcon.
    - apply IHis_merge.
...
(* skipping the rest for now. not really able to do all of them *)
