...
Proof.
  ...
  (* BEq & BLe *) simpl; repeat (rewrite optimize_0plus_sound); reflexivity.
Qed.
...
Fixpoint optimize_plus0 (a : aexp) : aexp :=
  match a with
  | ANum n => ANum n
  | APlus a1 (ANum 0) => optimize_plus0 a1
  | APlus a1 a2 => APlus (optimize_plus0 a1) (optimize_plus0 a2)
  | AMinus a1 a2 => AMinus a1 a2
  | AMult a1 a2 => AMult a1 a2
  end.

Theorem optimize_plus0_sound : forall (a : aexp),
  aeval a = aeval (optimize_plus0 a).
Proof.
  intros. induction a; try reflexivity.
  simpl. destruct a2 eqn:Ea2; simpl; rewrite IHa1; try reflexivity.
  - destruct n; try rewrite <- plus_n_O; reflexivity.
  - destruct a4; try destruct n; simpl; simpl in IHa2; try rewrite IHa2; reflexivity.
Qed.
...
Inductive bevalR : bexp -> bool -> Prop :=
  ...
  | E_BNot (bx : bexp) (b : bool) :
      (bevalR bx b) -> (bevalR (BNot bx) (negb b))
  | E_BAnd (bx1 bx2 : bexp) (b1 b2 : bool) :
      (bevalR bx1 b1) ->
      (bevalR bx2 b2) ->
      (bevalR (BAnd bx1 bx2) (andb b1 b2)).
...
Theorem XtimesYinZ_spec : forall st nx ny st',
  ...
  st' Z = nx * ny.
Proof.
  intros st nx ny st' HX HY He.
  inversion He. subst. simpl. apply t_update_eq. Qed.
...
| nw_Ass x n : no_whilesR (CAss x n)
| nw_Seq c1 c2 :
  (no_whilesR c1) ->
  (no_whilesR c2) ->
...
Proof.
  ...
  + intros H. induction c;
    ...
    try apply nw_Seq; try apply nw_If;
    try apply IHc1, H1; try apply IHc2, H2.
    - discriminate H.
  + intros H. induction H; try reflexivity;
      simpl; rewrite IHno_whilesR1, IHno_whilesR2; reflexivity.
Qed.
...
Proof.
  ...
  - inversion H. subst. rename H2 into H1. rename H3 into H2. clear H.
    destruct (IHc1 H1 st) as [st1 Hst1].
    destruct (IHc2 H2 st1) as [st2 Hst2].
    exists st2. apply (E_Seq c1 c2 st st1 st2); assumption.
  - inversion H. subst. rename H2 into H1. rename H4 into H2. clear H.
    destruct (IHc1 H1 st) as [st1 Hst1].
    destruct (IHc2 H2 st) as [st2 Hst2].
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | hp :: tp =>
    match hp with
    | SPush n => s_execute st (n :: stack) tp
    | SLoad x => s_execute st (st x :: stack) tp
    | SPlus => match stack with
               | n1 :: n2 :: ts => s_execute st ((n2 + n1) :: ts) tp
               | _ => s_execute st stack tp
               end
...
Lemma s_execute_seq :
  forall (prog1 prog2 : list sinstr) (st : state) (stack : list nat),
    s_execute st stack (prog1 ++ prog2)
  = s_execute st (s_execute st stack prog1) prog2.
Proof.
  intros prog1 prog2. induction prog1.
  + reflexivity.
  + intros. simpl.
    destruct a;
    repeat (try destruct stack; try apply IHprog1).
Qed.

Lemma s_compile_general :
  forall (e : aexp) (st : state) (stack : list nat),
  s_execute st stack (s_compile e) = [ aeval st e ] ++ stack.
Proof.
  intros e st. induction e; intros; simpl; try reflexivity;
    repeat (rewrite s_execute_seq);
    rewrite IHe1, IHe2; reflexivity.
Qed.  
(* Do think: what lemma should I prove to be able to use IH? *)

Theorem s_compile_correct : forall (st : state) (e : aexp),
  s_execute st [] (s_compile e) = [ aeval st e ].
Proof.
  intros. apply s_compile_general.
Qed.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_IfFalse : forall st st' b c1 c2 s,
      ...
      st =[ c2 ]=> st' / s ->
      st =[ TEST b THEN c1 ELSE c2 FI ]=> st' / s
  | E_SeqBreak : forall st st1 c1 c2,
      st =[ c1 ]=> st1 / SBreak ->
      st =[ c1;; c2 ]=> st1 / SBreak
  | E_SeqCont : forall st st1 st2 c1 c2 s2,
      st =[ c1 ]=> st1 / SContinue ->
      st1 =[ c2 ]=> st2 / s2 ->
      st =[ c1;; c2 ]=> st2 / s2
...
Theorem while_stops_on_break : forall b c st st',
  ...
  st =[ WHILE b DO c END ]=> st' / SContinue.
Proof.
  intros. constructor; assumption. Qed.
