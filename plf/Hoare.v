...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileTrue : forall st st' st'' b c,
      ...
      st' =[ WHILE b DO c END ]=> st'' ->
      st  =[ WHILE b DO c END ]=> st''
  | E_IfOneTrue : forall st st' b c,
      beval st b = true ->
      st =[ c ]=> st' ->
      st =[ IF1 b THEN c FI ]=> st'
  | E_IfOneFalse : forall st b c,
      beval st b = false ->
      st =[ IF1 b THEN c FI ]=> st
...
Theorem hoare_if1 : forall P Q b c,
  ...
  {{fun st => P st /\ ~ (bassn b st)}} (SKIP)%imp {{Q}} ->
  {{P}} (IF1 b THEN c FI)%imp {{Q}}.
Proof.
  intros. intros st st' Hp.
  inversion Hp; subst; intros.
...
Proof.
  apply hoare_if1.
  + intros st st' Ha Hpre.
    inversion Ha. subst. simpl.
    destruct Hpre as [Hpre1 Hpre2].
    rewrite Hpre1. apply t_update_eq.
  + intros st st' Ha [Hpre1 Hpre2].
    inversion Ha. subst.
    unfold bassn in Hpre2.
    simpl in Hpre2.
    apply eq_true_negb_classical in Hpre2.
    apply eqb_eq in Hpre2.
    omega.
Qed.
...
Theorem hoare_repeat : forall P c b,
  ...
  {{fun st => P st}} REPEAT c UNTIL b END {{fun st => P st /\ bassn b st}}.
Proof.
  intros P c b Hhoare st st' Hr Hp.
  remember (REPEAT c UNTIL b END)%imp as rcom eqn:Heqrcom.
  induction Hr; try (inversion Heqrcom); subst; clear Heqrcom.
  - split. apply (Hhoare st st'); try assumption.
    apply bexp_eval_true. assumption.
  - apply IHHr2. reflexivity. apply (Hhoare st st'); assumption.
Qed.
...
Lemma neq0_gt : forall (n : nat),
  n <> 0 -> 0 < n.
Proof.
  intros. unfold not in H. destruct n.
...
Proof.
  remember (REPEAT
    Y ::= X;;
    X ::= X - 1
  UNTIL X = 0 END) as rcom eqn:Heqrcom.
  unfold hoare_triple. intros.
  induction H; try (inversion Heqrcom); subst; clear Heqrcom.
  - split. simpl in H1. apply eqb_eq in H1. assumption.
    inversion H. subst.
    inversion H5. subst.
    inversion H7. subst. simpl in *.
    unfold t_update. simpl. assumption.
  - apply IHceval2. reflexivity.
    simpl in H1. apply beq_nat_false in H1.
    apply neq0_gt in H1. omega.
Qed.
...
Proof.
  ...
  + intros st st' Hc HP.
    exists empty_st.
    split.
    - inversion Hc. subst. inversion H0.
    - apply I.
  + unfold not. intros.
    unfold hoare_triple in H.
    simpl in H.
    induction H with (empty_st) (RError).
...
Proof.
  unfold hoare_triple.
  intros.
  destruct H with (st) (RNormal st).
  - apply E_AssertTrue.
    inversion H0. subst. assumption.
  - assumption.
  - inversion H2. inversion H3.
    inversion H0. subst. rename x into st. clear H3.
    exists st.
    destruct H with (st) (RNormal st).
