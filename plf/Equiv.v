...
Proof.
  ...
  - apply E_IfFalse; simpl; try apply negb_false_iff; assumption.
  - apply E_IfTrue; simpl; try apply negb_true_iff; assumption.
  - apply E_IfFalse; simpl; try apply negb_true_iff in H5; assumption.
  - apply E_IfTrue; simpl; try apply negb_false_iff in H5; assumption.
Qed.
(** [] *)
...
Proof.
  ...
  - unfold bequiv in Hb. rewrite Hb in H4. inversion H4.
...
Proof.
  unfold cequiv. intros. split; intros H; inversion H; subst.
  + inversion H2. subst.
...
Proof.
  unfold cequiv.
  intros. split; intros Hce; inversion Hce; subst;
    apply E_Seq with (st' := st'0);
      try apply H; try assumption;
...
Theorem CIf_congruence : forall b b' c1 c1' c2 c2',
  ...
  cequiv (TEST b THEN c1 ELSE c2 FI)
         (TEST b' THEN c1' ELSE c2' FI).
Proof.
  unfold bequiv, cequiv. split; intros; inversion H2; subst.
  - apply E_IfTrue.
...
Proof.
  ...
  - (* WHILE *)
    assert (Hbb: bequiv b (fold_constants_bexp b)). {
      apply fold_constants_bexp_sound.
    }
    destruct (fold_constants_bexp b) eqn:Heqb;
...
Proof.
  ...
  induction a; simpl; try rewrite IHa1, IHa2; try reflexivity.
...
Proof.
  ...
  induction b; simpl; try rewrite IHb1, IHb2;
...
Theorem vnu_subst_equiv : forall x1 x2 a1 a2,
  var_not_used_in_aexp x1 a2 ->
  cequiv (x1 ::= a1;; x2 ::= a2)
         (x1 ::= a1;; x2 ::= subst_aexp x1 a1 a2).
Proof.
  intros. apply CSeq_congruence; apply CAss_congruence.
  * apply refl_aequiv.
  * unfold aequiv. intros.
    induction a2; inversion H; simpl; auto.
    - simpl.
      apply eqb_string_false_iff in H1.
...
Proof.
  intro Contra. unfold cequiv in Contra.
  assert (Hskip: forall st, st =[ SKIP ]=> st) by (apply E_Skip).
  destruct Contra with (st := empty_st) (st' := empty_st).
  apply H0 in Hskip.
  apply loop_never_stops in Hskip.
  destruct Hskip.
Qed.
...
Proof.
  right. unfold not, cequiv, ptwice, pcopy. intros.
  assert (Htwice: empty_st =[ ptwice ]=> (Y !-> 1; X !-> 0)).
  { apply E_Seq with (X !-> 0). apply E_Havoc. apply E_Havoc. }
  assert (Hcopy: empty_st =[ pcopy ]=> (Y !-> 1; X !-> 1)).
  { apply E_Seq with (X !-> 1). apply E_Havoc. apply E_Ass. reflexivity. }
  apply H in Htwice.
  inversion Htwice. subst.
  inversion H5. subst.
  simpl in H6.
  inversion H2. subst.
  assert (Happ: (X !-> n) X = n). {
    apply t_update; constructor.
  }
  rewrite Happ in H6.
  remember (Y !-> n; X !-> n) as dict.
  assert (Hd: dict X = dict Y). {
    rewrite Heqdict. apply t_update; constructor.
  }
  assert (Hdx: dict X = 0). {
    rewrite H6. apply t_update; constructor.
  }
  assert (Hdy: dict Y = 1). {
    rewrite H6. apply t_update; constructor.
  }
...
Proof.
  ...
  + rewrite H2 in H0. simpl in H0.
    apply H, beq_nat_true, negb_false_iff. congruence.
  + apply IHceval2; auto.
    rewrite H3 in H0_.
    inversion H0_; subst.
    inversion H8; subst.
...
Proof.
  unfold cequiv. split; intros;
    inversion H; subst;
    unfold p1; unfold p2; try apply E_WhileFalse; auto.
  + apply p1_may_diverge in H6.
    - destruct H6.
    - unfold not. intros.
      inversion H3; subst.
      inversion H9; subst.
      ...
      inversion H0.
  + apply p2_may_diverge in H6.
    - destruct H6.
    - simpl in H2.
      apply negb_true_iff, beq_nat_false in H2.
      inversion H3; subst.
      assumption.
Qed.
...
Proof.
  ...
  assert (Hp3: (X !-> 2) =[ p3 ]=> (Z !-> 3; X !-> 0; Z !-> 1; X !-> 2)). {
    unfold p3. apply E_Seq with (Z !-> 1; X !-> 2).
    - apply E_Ass. reflexivity.
    - apply E_WhileTrue with (Z !-> 3; X !-> 0; Z !-> 1; X !-> 2).
      + reflexivity.
      + apply E_Seq with (X !-> 0; Z !-> 1; X !-> 2); apply E_Havoc.
      + apply E_WhileFalse. reflexivity.
  }
  assert (Hp4: (X !-> 2) =[ p4 ]=> (Z !-> 1; X !-> 0)). {
    unfold p4. apply E_Seq with (X !-> 0).
    - replace (X !-> 0) with (X !-> 0; X !-> 2).
      * apply E_Ass; reflexivity. * apply t_update_shadow.
    - apply E_Ass. reflexivity.
  }
  apply H in Hp3.
  inversion Hp3. subst.
  ...
  inversion H6. subst.
  simpl in *.
  remember (Z !-> 1; st') as dz1.
  remember (Z !-> 3; X !-> 0; Z !-> 1; X !-> 2) as dz2.
  assert (Hdz1: dz1 Z = 1). {
    rewrite Heqdz1. apply t_update_eq.
  }
  assert (Hdz2: dz2 Z = 3). {
    rewrite Heqdz2. apply t_update_eq.
  }
  congruence.
Qed.
...
Proof.
  unfold cequiv. split; intros;
    assert (Hd: (l2 !-> aeval st a2; l1 !-> aeval st a1; st)
        = (l1 !-> aeval st a1; l2 !-> aeval st a2; st)) by (
  ...
  + rewrite Hd.
    apply E_Seq with (l2 !-> aeval st a2; st); apply E_Ass.
    - reflexivity.
    - apply aeval_weakening. assumption.
  + rewrite <- Hd.
    apply E_Seq with (l1 !-> aeval st a1; st); apply E_Ass.
    - reflexivity.
    - apply aeval_weakening. assumption.
Qed.
...
Proof.
  ...
  split; intros Contra;
    assert (H0: empty_st =[ (X ::= 0)%imp ]=> (X !-> 0))
      by (constructor; reflexivity);
    assert (H42: empty_st =[ (X ::= 42)%imp ]=> (X !-> 42))
      by (constructor; reflexivity);
    assert (H0a: (X !-> 0) X = 0) by (constructor);
    assert (H42a: (X !-> 42) X = 42) by (constructor);
    try (apply Contra in H0; inversion H0);
    try (apply Contra in H42; inversion H42);
...
Proof.
  unfold capprox, cmin. intros. exfalso.
...
Definition zprop (c : com) : Prop := forall st st',
  st =[ c ]=> st'.

Theorem zprop_preserving : forall c c',
  zprop c -> capprox c c' -> zprop c'.
Proof.
  unfold capprox, zprop. intros. auto.
Qed.
