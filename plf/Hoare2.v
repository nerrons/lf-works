...
Proof.
  ...
  - instantiate (2 := (fun st => parity (st X) = parity m)).
    apply hoare_while.
    eapply hoare_consequence_pre. apply hoare_asgn.
    unfold assn_sub. simpl. intros st H. destruct H.
  ...
  - intros st [Hp HbX].
    rewrite <- Hp. symmetry. apply parity_lt_2.
    unfold not in *. intros. apply HbX.
    unfold bassn. simpl.
    destruct (st X). inversion H.
    destruct n. inversion H. inversion H1. reflexivity.
Qed.
...
(** **** Exercise: 3 stars, standard (Min_Hoare)  
  ...
  Y ::= b;;
    {{ X = a /\ Y = b /\ 0 = 0 }}
  Z ::= 0;;
    {{ Z = min a b - min (a-Z) (b-Z)
       /\ Z + X = a /\ Z + Y = b) }}
  WHILE ~(X = 0) && ~(Y = 0) DO
    {{ Z = min a b - min (a-Z) (b-Z)
       /\ Z + X = a /\ Z + Y = b)
       /\ ~(X = 0) /\ ~(Y = 0) }} ->>
    {{ Z = min a b - (min (a-Z-1) (b-Z-1) + 1)
       /\ Z + X = a /\ Z + Y = b }} ->>
    {{ Z + 1 = min a b - min (a-Z-1) (b-Z-1)
       /\ (Z + 1) + (X - 1) = a /\ (Z + 1) + (Y - 1) = b }}
  X := X - 1;;
    {{ Z + 1 = min a b - min (a-Z-1) (b-Z-1)
       /\ (Z + 1) + X = a /\ (Z + 1) + (Y - 1) = b }}
...
(* FILL IN HERE 
    ...
    WHILE ~(X = m) DO
        {{ Y = 1 + 2 + 2^2 + ... + 2^X /\ Z = 2^X }} ->>
        {{ Y + 2*Z = 1 + 2 + 2^2 + ... + 2^(X + 1) /\ 2*Z = 2^(X + 1)
           /\ ~(X = m) }}
      Z ::= 2 * Z;;
        {{ Y + Z = 1 + 2 + 2^2 + ... + 2^(X + 1) /\ Z = 2^(X + 1) }}
      Y ::= Y + Z;;
        {{ Y = 1 + 2 + 2^2 + ... + 2^(X + 1) /\ Z = 2^(X + 1) }}
      X ::= X + 1
        {{ Y = 1 + 2 + 2^2 + ... + 2^X /\ Z = 2^X }}
    END
      {{ Y = 1 + 2 + 2^2 + ... + 2^X /\ Z = 2^X /\ X = m }} ->>
      {{ Y = 2^(X+1) - 1 /\ X = m }} ->>
      {{ Y = 2^(m+1) - 1 }}
...
Proof.
  ...
  + intros. intros st HP'.
    ...
    apply (H st (X !-> st Y + 1; st)) in HP'.
    - unfold t_update in HP'. simpl in *. omega.
...
Proof.
  ...
  + apply hoare_asgn.
  + intros P' H st HP'.
    unfold hoare_triple in H. unfold assn_sub.
    apply (H st (X !-> aeval st a; st)) in HP'.
    - assumption.
    - apply E_Ass. reflexivity.
Qed.
(** [] *)
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : string),
  ...
  P ->> havoc_pre X Q.
Proof.
  intros. intros st HP. unfold havoc_pre. intros.
  unfold hoare_triple in H.
  apply H with st.
  - apply E_Havoc.
  - assumption.
...
Proof.
  ...
  - destruct (st X); try congruence.
    rewrite <- H.
    assert (Hf: S n * real_fact (S n - 1) = real_fact (S n)). {
      simpl. rewrite <- minus_n_O. reflexivity.
    }
    rewrite <- Hf. rewrite mult_assoc. reflexivity.
  - simpl in H. omega.
Qed.
...
Proof.
  destruct n eqn:En; intros H.
  + omega.
  + unfold pred.
    replace (S n0 + 1) with (S (S n0)) by omega.
    replace (fib (S n0) + fib n0) with (fib (S (S n0))) by reflexivity.
    reflexivity.
Qed.
...
Definition dfib (n : nat) : decorated :=
  ...
  Z ::= 1
  {{ fun st => st X = 1 /\ st Y = 1 /\ st Z = 1 }} ->>
  {{ fun st => st Y = fib (st X - 1) /\ st Z = fib (st X) /\ st X > 0}};;
  WHILE ~(X = n + 1) DO
    {{ fun st => st Y = fib (st X - 1) /\ st Z = fib (st X) /\ st X > 0
       /\ ~(st X = n + 1) }} ->>
    {{ fun st => st Z = fib (st X) /\ st Z + st Y = fib (st X + 1) /\ st X + 1 > 0}}
    T ::= Z
    {{ fun st => st T = fib (st X) /\ st Z + st Y = fib (st X + 1) /\ st X + 1> 0}};;
    Z ::= Z + Y
    {{ fun st => st T = fib (st X) /\ st Z = fib (st X + 1) /\ st X + 1 > 0 }};;
    Y ::= T
    {{ fun st => st Y = fib (st X) /\ st Z = fib (st X + 1) /\ st X + 1 > 0 }};;
    X ::= X + 1
    {{ fun st => st Y = fib (st X - 1) /\ st Z = fib (st X) /\ st X > 0 }}
  END
  {{ fun st => st Y = fib (st X - 1) /\ st Z = fib (st X) /\ st X > 0
